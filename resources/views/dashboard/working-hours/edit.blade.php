...
@section('title')
    EDit {{ $workingHour->day }} Working Hour
@endsection
...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('dashboard.working-hours.update' ,$workingHour) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="row mb-3">
                        <div class="col-4">
                            ...
                            <select class="form-control" name="day" id="day">
                                ...
                                @foreach(\App\Enums\WorkingDays::cases() as $day)
                                    <option
                                        @selected(old('day', $workingHour->day) == $day->value) value="{{ $day }}">{{ $day }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-4">
                            ...
                            <select class="form-control" name="from" id="from">
                                ...
                                @foreach($times as $time)
                                    <option
                                        @selected(old('from', $workingHour->from) == $time) value="{{ $time }}">{{ $time }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-4">
                            ...
                            <select class="form-control" name="to" id="to">
                                ...
                                @foreach($times as $time)
                                    <option
                                        @selected(old('to', $workingHour->to) == $time) value="{{ $time }}">{{ $time }}</option>
                                @endforeach
                            </select>
